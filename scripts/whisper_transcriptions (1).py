        ...
        if winget_packages.exists():
            for p in winget_packages.iterdir():
                if p.name.lower().startswith("gyan.ffmpeg_"):
                    # search for any bin/ffmpeg.exe under this package
...
# If dry-run requested, report which manifest entries are missing audio and exit
if args.dry_run:
    print("\n--- Dry run: comparing manifest to found audio files ---")
    missing = []
    # Respect --limit when checking manifest rows
